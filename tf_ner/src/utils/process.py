...
def convert_examples_to_features(examples, max_seq_length, tokenizer, label_map=None):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        label_ids = []
        token_ids, segment_ids = tokenizer.encode(example.text_a, maxlen=max_seq_length)
...
def process_data(data_dir, examples,  tokenizer, max_seq_length, label_list=None, name='train'):
    ...
    input_ids = sequence_padding(input_ids, length=max_seq_length)
    token_type_ids = sequence_padding(token_type_ids, length=max_seq_length)
    label_id = sequence_padding(label_id, length=max_seq_length)
