...
class Bert_gru_ner(nn.Layer):
    def __init__(self, config):
        super(Bert_gru_ner, self).__init__()
        self.config = config
        self.ernie = ErnieModel.from_pretrained(config.MODEL_NAME)

        self.drop_out = nn.layer.Dropout(config.drop_out)
        self.gru = nn.GRU(self.ernie.config['hidden_size'], config.hidden_size, num_layers=2, direction='bidirect')
        self.classifier = nn.layer.Linear(config.hidden_size*2, config.num_classes+2)
    ...
    def forward(self, batch):
        ...
        gru, _  = self.gru(drop_out)

        classifier = self.classifier(gru)
