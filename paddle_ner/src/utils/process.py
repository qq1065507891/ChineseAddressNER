...
def convert_examples_to_features(examples, config, max_seq_length, tokenizer, label_map=None):
...
def process_data(data_dir, config, examples,  tokenizer, max_seq_length, label_list=None, name='train'):
    ...
    all_seq_length = paddle.to_tensor(seq_length, dtype='int64')
    all_label_ids = paddle.to_tensor(label_id, dtype='int64')

    data = TensorDataset([all_input_ids, all_token_type_ids, all_seq_length, all_label_ids])
    # data = TensorDataset([input_ids, token_type_ids, seq_length, label_id])
